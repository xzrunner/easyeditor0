...
void EditPanelImpl::OnMouse(wxMouseEvent& event)
{
	...
	auto& io = facade::EasyGUI::Instance()->ctx.io;
	...
	if (event.LeftDown())
	{
		...
		egui::feed_event(io, egui::InputEvent(egui::InputType::MOUSE_LEFT_DOWN, proj.x, proj.y));
	}
	else if (event.LeftUp())
	{
		m_edit_op->OnMouseLeftUp(x, y);
		egui::feed_event(io, egui::InputEvent(egui::InputType::MOUSE_LEFT_UP, proj.x, proj.y));
	}
	else if (event.RightDown())
	{
		m_edit_op->OnMouseRightDown(x, y);
		egui::feed_event(io, egui::InputEvent(egui::InputType::MOUSE_RIGHT_DOWN, proj.x, proj.y));
	}
	else if (event.RightUp())
	{
		m_edit_op->OnMouseRightUp(x, y);
		egui::feed_event(io, egui::InputEvent(egui::InputType::MOUSE_RIGHT_UP, proj.x, proj.y));
	}
	else if (event.Moving())
	{
		m_edit_op->OnMouseMove(x, y);
		egui::feed_event(io, egui::InputEvent(egui::InputType::MOUSE_MOVE, proj.x, proj.y));
	}
	else if (event.Dragging())
	{
		m_edit_op->OnMouseDrag(x, y);
		egui::feed_event(io, egui::InputEvent(egui::InputType::MOUSE_DRAG, proj.x, proj.y));
	}
	else if (event.LeftDClick())
	{
		m_edit_op->OnMouseLeftDClick(x, y);
		egui::feed_event(io, egui::InputEvent(egui::InputType::MOUSE_LEFT_DCLICK, proj.x, proj.y));
	}
	else if (event.GetWheelRotation())
	{
		int dir = event.GetWheelRotation();
		m_edit_op->OnMouseWheelRotation(x, y, dir);
		egui::feed_event(io, egui::InputEvent(egui::InputType::MOUSE_WHEEL_ROTATION, proj.x, proj.y, dir));
	}
